// /js/language.js
// Version: Added missing keys and fixed copyright handling
...
const translations = {
    vi: {
        ...
        form_label_phone: "Số điện thoại", // Key đã có nhưng bị báo lỗi? Kiểm tra lại HTML
        ...
        ai_guide_health_ops1_revised: "Nhận dạng Hình ảnh Y tế: Khi AI phân tích ảnh X-quang hay CT scan, nó đối chiếu các đặc điểm ảnh với hàng triệu ảnh đã được gán nhãn bởi chuyên gia (bình thường/có dấu hiệu bệnh lý) để phát hiện các dấu hiệu bất thường tiềm ẩn.",
        ...
        ai_guide_finance_ops1_revised: "Phát hiện Bất thường (Anomaly Detection): Hệ thống chống gian lận AI liên tục \"học\" các mẫu giao dịch thông thường. Khi một giao dịch đi chệch khỏi các mẫu này (ví dụ: vị trí địa lý lạ, số tiền đột biến, tần suất bất thường), AI sẽ đưa ra cảnh báo để xem xét kỹ hơn.",
        ai_guide_finance_ops2_revised: "Phân loại & Dự đoán Rủi ro: Mô hình chấm điểm tín dụng AI phân loại khách hàng vào các nhóm rủi ro khác nhau bằng cách \"học\" mối tương quan giữa hàng loạt yếu tố (thu nhập, lịch sử tín dụng, tỷ lệ nợ,...) và khả năng trả nợ đã được ghi nhận trong quá khứ.",
        ...
        ai_guide_retail_mindset3_revised: "Nhạy bén với Dữ liệu Khách hàng: Coi mọi điểm chạm của khách hàng (lượt xem trang, click, giỏ hàng bị bỏ quên, đánh giá sản phẩm) là nguồn dữ liệu vô giá để AI khai thác và đưa ra gợi ý hành động.",
    ...
    document.querySelectorAll('[data-lang-key]').forEach(element => {
        ...
        if (translation !== undefined) {
            ...
            if (tagName === 'META' && nameAttr === 'description') {
                ...
                // Use main key as fallback alt text if specific alt key is missing
                element.alt = altTranslation !== undefined ? altTranslation : translation;
                ...
                if (hasMarkdown) {
                    ...
                    // If no markdown, check if element already contains HTML structure
                    const containsHtmlTags = /<[a-z][\s\S]*>/i.test(element.innerHTML);
                    // Only set textContent if the element doesn't already have complex children
                    // or if the translation is clearly intended to replace everything.
                    // This prevents accidentally wiping out icons inside buttons/links.
                    if (!containsHtmlTags || element.children.length === 0 || tagName === 'P' || tagName.startsWith('H')) {
                         element.textContent = finalTranslation; // Prefer textContent for safety
                    } else {
                         // Attempt to update only the main text node if possible (more complex)
                         // Find the first text node child and update it? Risky.
                         // For now, log a warning if we skip updating complex elements with plain text.
                         console.warn(`[Language] Skipped updating complex element for key "${key}" with plain text to avoid breaking structure.`);
                    }
                }
            }
...
function setLanguage(lang) {
    ...
    // Apply translations *before* updating UI elements that might depend on text width/content
    ...
    // Trigger dynamic content updates AFTER translations are applied
    ...
    // Re-highlight menu *after* text content might have changed
    if (typeof initializeActiveMenuHighlighting === 'function') {
         ...
         if (headerElement) {
              ...
              // Use setTimeout to ensure DOM updates from applyTranslations are rendered
              setTimeout(() => initializeActiveMenuHighlighting(headerElement), 0);
         }
    }
}
